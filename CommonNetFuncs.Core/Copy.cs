...
public static class Copy
{
	...
	private static Func<object, object?, int, int, object?> GetOrAddFunctionFromDeepCopyCache((Type sourceType, Type destType) key)
	{
		if (DeepCopyCacheManager.IsUsingLimitedCache() ?
			DeepCopyCacheManager.GetLimitedCache().TryGetValue(key, out Func<object, object?, int, int, object?>? function) :
	...
	private static Dictionary<string, (Delegate Set, Delegate Get)> GetOrAddFunctionFromCopyCache<TSource, TDest>()
	{
		...
		return isLimitedCache
			? CopyCacheManager.GetOrAddLimitedCache(key, _ => CreatePropertyMappingsForCache<TSource, TDest>())
			: CopyCacheManager.GetOrAddCache(key, _ => CreatePropertyMappingsForCache<TSource, TDest>());
	}
	...
	public static void CopyPropertiesTo<T, UT>(this T source, UT dest, bool useCache = true)
	{
		...
		if (useCache)
		{
			...
			foreach ((Action<UT, object?> Set, Func<T, object?> Get) in GetOrCreatePropertyMaps<T, UT>().Select(x => x.Value))
			{
				Set(dest, Get(source));
			}
		}
	...
	public static T CopyPropertiesToNew<T>(this T source, bool useCache = true) where T : new()
	{
		...
		if (useCache)
		{
			foreach ((Action<T, object?> Set, Func<T, object?> Get) in GetOrCreatePropertyMaps<T, T>().Select(x => x.Value))
			{
				Set(dest, Get(source));
			}
		}
	...
	public static UT? CopyPropertiesToNewRecursive<T, UT>(this T source, int maxDepth = -1, bool useCache = true) where UT : new()
	{
		return EqualityComparer<T?>.Default.Equals(source, default)
			? default
			: useCache
			? CopyPropertiesToNewRecursiveExpressionTrees<T, UT>(source, maxDepth)!
			: typeof(IEnumerable).IsAssignableFrom(typeof(T)) && typeof(IEnumerable).IsAssignableFrom(typeof(UT)) && (typeof(T) != typeof(string)) && (typeof(UT) != typeof(string))
				? (UT?)CopyCollection(source!, typeof(UT), maxDepth) ?? new()
				: (UT?)CopyObject(source!, typeof(UT), 0, maxDepth) ?? new();
	}
	...
	private static Expression? CreateValueCopyExpression(Expression sourceValue, Expression destProperty, Type sourceType, Type destType, ParameterExpression depthParam, ParameterExpression maxDepthParam)
	{
		...
		return destType.IsAssignableFrom(sourceType) ? Expression.Assign(destProperty, Expression.Convert(sourceValue, destType)) : (Expression?)null;
	}
	...
	private static object? CopyItemRuntime(object? item, Type destType, int depth, int maxDepth)
	{
		...
		return itemType.IsSimpleType() || itemType == destType ? item : CopyObjectRuntime(item, destType, depth, maxDepth);
	}
