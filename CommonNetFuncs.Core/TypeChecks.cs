...
public static class TypeChecks
{
	...
	public static bool IsNumericType(this Type? type)
	{
		...
		return NumericTypeCache.GetOrAdd(type, x =>
		{
			...
			return Type.GetTypeCode(x) is TypeCode.Int32 or TypeCode.Int64 or TypeCode.Double or TypeCode.Decimal or TypeCode.Int16 or
				TypeCode.Byte or TypeCode.SByte or TypeCode.UInt16 or TypeCode.UInt32 or TypeCode.UInt64 or TypeCode.Single;
		});
	}
	...
	public static bool IsReadOnlyCollectionType(this Type type)
	{
		...
		return ReadOnlyCollectionTypeCache.GetOrAdd(type, x =>
		{
			if (x.IsGenericType)
			{
				...
				if (genericType == typeof(IReadOnlyCollection<>) || genericType == typeof(IReadOnlyList<>) || genericType == typeof(ReadOnlyCollection<>))
				{
					return true;
				}
			...
			foreach (Type interfaceType in x.GetInterfaces())
			{
				...
				{
					Type genericInterfaceType = interfaceType.GetGenericTypeDefinition();
					if (genericInterfaceType == typeof(IReadOnlyCollection<>) || genericInterfaceType == typeof(IReadOnlyList<>))
					{
						return true;
					}
				}
