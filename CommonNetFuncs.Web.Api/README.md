...
- [CommonNetFuncs.Web.Api](#commonnetfuncswebapi)
  ...
  - [MsgPack](#msgpack)
    ...
    - [DateTime, TimeSpan, and DateTimeOffset JSON Converters](#datetime-timespan-and-datetimeoffset-json-converters)
      - [AddMsgPackDateTimeJsonConverters](#addmsgpackdatetimejsonconverters)
...
### DateTime, TimeSpan, and DateTimeOffset JSON Converters

ASP.NET Core's default JSON serializer doesn't have built-in support for serializing `TimeSpan` as human-readable strings, and it treats `DateTime`/`DateTimeOffset` differently depending on their `Kind` property. When using MessagePack's `DateTimesAsStrings` configuration (which serializes these types as ISO 8601 strings for better interoperability), the JSON converters ensure consistent behavior across both MessagePack and JSON serialization paths.

**Key behaviors:**

- `DateTime` with `DateTimeKind.Unspecified` is normalized to UTC before serialization, matching MessagePack formatter behavior
- `TimeSpan` serializes to the constant format (e.g., `"01:30:00"`) instead of arrays
- `DateTimeOffset` serializes to ISO 8601 format with timezone offset (e.g., `"2026-04-14T12:00:00+05:00"`)

<details>
<summary><h3>Usage Examples</h3></summary>

#### AddMsgPackDateTimeJsonConverters

Registers the JSON converters with ASP.NET Core's default JSON serialization options. Call this in `Program.cs` during service configuration when using `MsgPackSerializerConfig.DateTimesAsStrings` to ensure consistent behavior when MessagePack is converted to JSON.
...
// Also configure MsgPack output to use DateTimesAsStrings
...
RouteGroupBuilder apiGroup = app.MapGroup("/Api")
    ...
    .WithMsgPackOutput(
        MsgPackSerializerConfig.DateTimesAsStrings
            .WithSecurity(MessagePackSecurity.UntrustedData)
            .WithCompression(MessagePackCompression.Lz4BlockArray))
    .WithTags("API");

// Now both JSON and MessagePack paths handle DateTime/TimeSpan/DateTimeOffset consistently
apiGroup.MapPost("/events", (MyEventDto dto) => Results.Ok(dto));
```

**Example request/response with TimeSpan:**

```cs
public record MyEventDto(string Name, TimeSpan Duration, DateTime StartTime);

// JSON serialization (automatic)
{
  "name": "Meeting",
  "duration": "01:30:00",
  "startTime": "2026-04-14T12:00:00.0000000Z"
}

// MessagePack serialization (via Accept: application/x-msgpack)
// Same format internally
```

**Alternatively, if you prefer manual control:**

Use `MsgPackSerializerConfig.GetJsonSerializerOptionsWithDateTimeConverters()` to get a `JsonSerializerOptions` instance with the converters pre-configured for use elsewhere:

```cs
var options = MsgPackSerializerConfig.GetJsonSerializerOptionsWithDateTimeConverters();
var json = System.Text.Json.JsonSerializer.Serialize(myObject, options);
```
