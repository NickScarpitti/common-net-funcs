...
public class PrioritizedEndpointQueueService : IPrioritizedEndpointQueueService, IDisposable
{
    ...
    private PrioritizedEndpointQueue GetOrCreateQueue(string endpointKey)
    {
        ...
        {
            NLog.Logger logger = serviceProvider.GetService<NLog.Logger>() ?? NLog.LogManager.GetCurrentClassLogger();
            logger.Info("Creating new prioritized queue for endpoint: {EndpointKey}", key);
