...
public static class ChangeUrls
{
	...
	public static bool ChangeUrlsInWordDoc(Stream fileStream, string newUrl, string urlToReplace, bool replaceAll = true)
	{
		...
		try
		{
			...
			if (mainPart != null)
			{
				// .ToList() is needed to avoid "Collection was modified; enumeration operation may not execute." error
				foreach (HyperlinkRelationship? hyperlink in mainPart.HyperlinkRelationships.Where(hyperlink => string.Equals(hyperlink.Uri.ToString(), urlToReplace, StringComparison.InvariantCultureIgnoreCase)).ToList())
				{
