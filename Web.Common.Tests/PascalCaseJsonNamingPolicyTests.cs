...
public sealed class PascalCaseJsonNamingPolicyTests
{
    ...
    [InlineData("ID")]
    [InlineData("APIKey")]
    public void ConvertName_WhenGivenPascalCase_ReturnsUnmodified(string input)
    {
        // Act
        string result = _sut.ConvertName(input);
    ...
    [InlineData("$price")]
    [InlineData("_id")]
    public void ConvertName_WhenStartingWithNonLetter_ReturnsUnmodified(string input)
    {
        // Act
        string result = _sut.ConvertName(input);
