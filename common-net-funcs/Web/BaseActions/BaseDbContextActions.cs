...
public class BaseDbContextActions<T, UT> : IBaseDbContextActions<T, UT> where T : class where UT : DbContext
{
    ...
    /// <summary>
    /// Get individual record by the primary key. If using a compound primary key, use an object of the same class to be returned with the primary key fields populated
    /// </summary>
    ...
    /// <summary>
    /// Gets all records from the corresponding table that satisfy the conditions of the linq query expression. Same as running a SELECT * WHERE <condition> query
    /// </summary>
    ...
    /// <summary>
    /// Gets first record from the corresponding table that satisfy the conditions of the linq query expression. Same as running a SELECT * WHERE <condition> LIMIT 1 or SELECT TOP 1 * WHERE <condition> LIMIT 1 query
    /// </summary>
